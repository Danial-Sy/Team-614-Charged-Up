...
public class Dance extends CommandBase {
  ...
  public void execute() {
    ...
    if (currentTime < endTime) {
        if (isRotatingLeft) {
            RobotContainer.driveTrainSubsystem.arcadeDrive(Constants.MOTOR_ZERO_SPEED, ROTATION_SPEED);
        } else {
          RobotContainer.driveTrainSubsystem.arcadeDrive(Constants.MOTOR_ZERO_SPEED, -ROTATION_SPEED);
        }
      ...
      RobotContainer.driveTrainSubsystem.arcadeDrive(Constants.MOTOR_ZERO_SPEED, Constants.MOTOR_ZERO_SPEED); // stop the robot at the end of the dance
    }
}
